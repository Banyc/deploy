...
13. Under the default `failure_policy: rollback_changed`, compensate every server already advanced by this deployment. Compensation uses a compare-and-swap and restores a server only if `current` still names the generation created by this attempt. The terminal STATUS is then DERIVED by the semantic kernel from the per-slot outcome evidence (never a stored boolean — the `all_restored`/compensation-count boolean is gone): an attempt with failures is `failed_rolled_back` iff NO slot's outcome shows an `Advanced` transition — no slot PROVABLY ON the new state (a compensated slot, a never-advanced slot, or an advance-unknown slot — including an uncompensated failure whose compensation did not fully succeed, which rides the rolled-back terminal's compensation report as the slot's `Failed` outcome); it is `degraded` whenever any slot is still PROVABLY ON the new state (a retained `Activated` advance — e.g. under `leave_changed`, or an `Activated` slot the failure-policy pass could not reverse). An optional `leave_changed` policy may retain successful advances deliberately: the retained advances stay `Activated`, so any failure with an earlier advanced batch is `degraded`; a `leave_changed` failure with NOTHING advanced (a batch-0 pre-swap failure, the later batches skipped) leaves no slot on the new state and is therefore `failed_rolled_back` like any other never-advanced failure.
...
```json
{
  ...
  "intent_digest": "92594bd379f377e0b64e740c51f0b5cdbb551ae5a4753f693928c709ae59b8f5",
  "outcomes": [],
