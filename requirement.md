...
ledger line     = one physical JSONL line of `targets/<target>/ledger.jsonl`.
                  ...
                  slot ROW ARRAY — every slot's plan-minted result and its
                  Deploy/Inherit action, each row OWNING its slot id, in
                  DEPLOYMENT ORDER (never sorted by id) — the behavior
                  digest, and the pre-push states — no status, no outcomes)
                  and the TERMINAL
...
outcome         = one slot's terminal result (SlotResult): its STRUCTURAL
                  execution-state body (`activated` / `restored` / `skipped` /
                  `failed_before_advance` / `failed_after_advance` /
                  `indeterminate`) with each state's own post-mutation
                  observation (and the failed states' operation error) —
                  compensation is a STATE (`restored`), never a boolean.
                  Outcomes live in the terminal event's per-slot ROW ARRAY;
                  there is no separate outcomes file.
...
and the aggregate remote publish (`HeldSlotLock::publish_release(&ValidatedReleaseBundle)`) re-derive the
canonical digest from the record's own content (slot snapshot,
...
hatch). REPUBLISHING against an already-present remote release directory content-verifies the EXISTING remote
`release.json` the same way before treating it as the same release: the aggregate publish re-derives the
...
3. **Release snapshots** — `mapping.toml`, `behavior.json` beside the release record.
   ...
   *Guarantee*: atomic create-or-compare writes (`store.write_atomic_cas`: temp file + rename for atomicity; existing content must match byte-for-byte or the write fails); remotely, the release is published as ONE AGGREGATE BUNDLE (`ValidatedReleaseBundle` — complete by construction, built from the semantically validated release) by a SINGLE publish operation (`HeldSlotLock::publish_release`): every member (`release.json`, `behavior.json`) is written into a UNIQUE SIBLING staging directory (`releases/<id>.partial-<nonce>`), the WHOLE bundle is verified there (the release record identity recomputed from its own content, the behavior digest against the record provenance), fsynced, and then ATOMICALLY INSTALLED by renaming the staging directory into the final release directory — the final release directory is either wholly absent or complete and readable, never a partial directory (a crash or fault at any stage leaves at most a disposable staging sibling). An existing release directory is verified as the WHOLE bundle before it is trusted (idempotent re-publication); a corrupted or partial existing directory fails closed, never silently replaced. There is no capacity snapshot: capacity headroom is live per-server configuration read from the caller's current `deploy.toml`.
...
13. Under the default `failure_policy: rollback_changed`, compensate every server already advanced by this deployment (an `Advanced` or `failed_after_advance` execution — the ADVANCE-REQUIRED set): a successful compensation FLIPS the slot's execution state to `restored` (compensation is a TRANSITION between states, never a boolean). Compensation uses a compare-and-swap and restores a server only if `current` still names the generation created by this attempt. The terminal STATUS is then DERIVED by the semantic kernel from the per-slot outcome evidence through the ONE per-slot classifier (`SlotDelta` — the observed post-state vs the intent's pre-push and DESIRED generations): an attempt with failures is `failed_rolled_back` iff EVERY slot's delta is UNCHANGED (observed == pre-push: a restored slot, a skipped slot, or a pre-swap failure still at its pre-push state); it is `degraded` whenever ANY slot's delta is DESIRED/DIVERGED/UNKNOWN (a retained `Activated` advance — e.g. under `leave_changed` — or an UNCOMPENSATED POST-ADVANCE failure, whose recorded observation is the desired generation it was advanced to and which the OLD transition-based rule could wrongly call rolled-back: the review's P1 fix — a Degraded terminal ALWAYS carries at least one remaining change). An optional `leave_changed` policy may retain successful advances deliberately: the retained advances stay `activated`, so any failure with an earlier advanced batch is `degraded`; a `leave_changed` failure with NOTHING advanced (a batch-0 pre-swap failure, the later batches skipped) leaves every slot at its pre-push state and is therefore `failed_rolled_back` like any other never-advanced failure.
...
15. After every slot's server verifies, write an idempotent, write-once commit marker under each participating server's mutation lock (exclusive create; an existing marker must match byte-for-byte). The marker carries the deployment ID, the generation, and the full placement-slot set of the commit. If this metadata phase is interrupted by a transient failure, mark the attempt `pending_commit`; the next push reconciles it before its own no-op check. Reconciliation also covers INTENT-ONLY ledger entries — the intent durable (persisted before mutation, step 14) but the terminal event never appended (a crash between `append_intent` and the finalize marker). Under the STRICTLY-LINEAR successful history there is AT MOST ONE such pending attempt at any time (a new intent is refused while one is unresolved), and recovery is a CALLER of the one kernel state machine — no second authority, no bypass. It verifies that every recorded participant slot still belongs to the target and that each slot's current generation still equals the generation the attempt recorded (fresh status reads), and only then writes the missing markers (under each server's mutation lock, with the original deployment ID) and finalizes the attempt through the SAME replay-safe finalizer the normal success path uses (step 16) — which REQUIRES the attempt's parent to still be the target's successful head (the finalizer always checks `intent.parent == last successful` and the atomic append gate re-checks it). A recovered attempt that can no longer finalize `successful` on the live state — a membership or generation mismatch, a drifted head, or a conflicting marker — is finalized through the ONE per-slot classifier: `degraded` when the evidence carries at least one remaining change (a `Degraded` terminal whose reason carries the refusal source), or `failed_rolled_back` when every slot's backend-observed state is back at its pre-push state (a Degraded terminal with NO remaining change is unrepresentable — never a `Degraded` that hides the emptiness: the review's P1 rule). Never `successful`, never stranded, never silently ignored. The degraded terminal's per-slot outcomes record the slots' BACKEND-OBSERVED live state, re-read under each selected slot's mutation lock BEFORE the terminal is decided (only a successful status + assignment read yields a `Known` observation; only a successful status read showing no `current` yields `KnownAbsent`; a read failure yields `Unknown` with the preserved error) — the plan's frozen DESIRED snapshot is never converted into an observed fact (a plan may create desired facts, but only a successful backend read may create observed facts, and a later rollback/`remaining_changes` reference read must never treat a fabricated desire as live-server truth); a slot remains a change iff its observed state differs from its pre-push state or is unknown. If the finalization CANNOT finish right now (a TRANSIENT non-finalization: lock acquisition or marker-write contention — including the degraded path's own evidence-read lock acquisition, where a truthful terminal cannot be built without the backend read), the attempt stays intent-only and the push is REFUSED with a conflict ("a previous deployment is still pending") — a push that cannot finish the previous pending attempt NEVER plans a second intent on top, even for disjoint groups, and a recovered attempt can never overlay a newer head's inherited entries on the logical history. APPEND THE TERMINAL EVENT LAST, so a crash mid-finalization leaves the entry terminal-less and therefore re-eligible. The verification is read-only; recovery never reactivates or restarts healthy servers. An existing marker whose content differs (an integrity conflict — a concurrent controller recorded a different fact, or the remote state diverged) is likewise NOT transient: the conflicting marker is left untouched and the attempt is finalized `degraded` (terminal only), never stranded `pending_commit` forever. Only transient failures — lock acquisition, status reads, or transport-level marker writes — leave the attempt `pending_commit` for a later retry rather than falsely reporting `successful` or `degraded`.
16. Only an attempt whose commit markers are complete becomes `successful`. Both the normal success path and recovery finalize through ONE replay-safe finalizer that REQUIRES `intent.parent == current successful head` (verified both in an explicit pre-check and INSIDE the pure state machine's strict-linear state, `apply_event`) and APPENDS THE TERMINAL `Successful` EVENT to the target's ledger (one atomic line, `store.append_terminal`; replay-idempotent — a repeated finalize for the same deployment id is a no-op). The lineage is enforced at INTENT-APPEND TIME (the authoritative gate): every ordinary intent's parent must equal the current successful head AT APPEND TIME and at most ONE unresolved intent may exist — so the successful history is STRICTLY LINEAR (each `Successful` entry's parent is its predecessor head, and its inherited entries were validated against that head's snapshot when it was appended: `resolve_snapshot` always derives from a head whose inherited entries were valid at append time). A head that drifted before finalization makes the plan STALE: the finalizer refuses and the attempt ends `degraded`, never `successful`. The terminal-time one-parent scan remains as a defensive re-check; a checkpoint-retained boundary entry (the anchor) is the model's ONE exception — it may reference a parent outside the retained window (validated at its original append) and must itself be `successful`. The terminal event carries the status; a SUCCESSFUL terminal is PAYLOAD-FREE, bound to its intent by its canonical `intent_digest` — the deployment's snapshot IS the intent's resulting slot table, RESOLVED on demand and keyed by its DEPLOYMENT ID: `deploy push <target> <deployment-id>` restores exactly that deployment's stored state, and there is no separate snapshot log and no `refs/last-successful` ref (the latest successful entry is DERIVED from the ledger).
...
- Never infer deployment success from the local plan; reconcile actual generations and commit markers, and record every attempt (statuses: `successful`, `pending_commit`, `failed_preflight`, `failed_rolled_back`, `degraded`) with per-slot outcomes — the STRUCTURAL execution states `activated` / `restored` / `skipped` / `failed_before_advance` / `failed_after_advance` / `indeterminate` (compensation is a STATE — `restored` — never a flag; a slot that advanced and was not restored is `failed_after_advance`, always a remaining change).
