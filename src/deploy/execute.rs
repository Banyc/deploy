...
//! [`ExecutionOutcome`] the commit phases ([`crate::deploy::commit`]) consume.
...
//! observed post-state via [`crate::deploy::failure::record_never_advanced_outcomes`].

use crate::config::SlotConfig;
use crate::deploy::batching::BatchRun;
use crate::deploy::preflight::PreflightOutcome;
use crate::deploy::push::PushContext;
use crate::error::Result;
use crate::identity::SlotId;
use crate::ledger::{DeploymentStatus, SlotAttemptState, SlotResult};
...
/// [`crate::deploy::batching`]; the failure-policy pass in
/// [`crate::deploy::failure`]; the commit-marker / status decision in
/// [`crate::deploy::status`]; the post-mutation observation in
/// [`crate::deploy::results`].
pub(crate) fn run_execution(
    ...
    // [`crate::deploy::batching`]; the outcome is the per-slot results plus
    ...
    // 13 & 14. The FAILURE-POLICY PASS lives in [`crate::deploy::failure`]:
    ...
    let status = super::failure::apply_failure_policy(
        had_failure,
        failure_policy,
        &outcome.assignments,
        members,
    ...
    let (actual_servers, actual_observations) =
        super::results::observe_actual_servers(&outcome.assignments, helpers);
    ...
    // desired generation. The fix-up lives in [`crate::deploy::failure`]
    // (`record_never_advanced_outcomes`).
    super::failure::record_never_advanced_outcomes(
        &mut results,
        &actual_observations,
        &never_advanced,
    );
...
pub(crate) mod tests {
    ...
    //! [`crate::deploy::push::push_inner`] with the shared harnesses from
