...
//! ([`crate::deploy::execute`], [`crate::deploy::commit`]) consume; the
...
use crate::identity::{
    BehaviorContract, DeploymentId, GenerationId, OperationId, ReleaseId, SlotId, TargetName,
    TreeDigest, VariantName,
};
use crate::ledger::recovery::reconcile_pending_commits;
use crate::ledger::{
    self, BehaviorIndex, DeploymentIntent, DeploymentPlan, DesiredGeneration, IntentSlot,
    NonEmptySlotTable, Observation, ObservationError, PreviousGeneration, PushRef,
    SlotAttemptState, SlotPlan,
};
...
/// The DIRECT-RELEASE MEMBERSHIP GATE ([`crate::deploy::push::push`] step
...
/// [`push_inner`](crate::deploy::push::push_inner) is ever touched. For a
...
pub(crate) fn gate_direct_release_membership(
    ...
    if let crate::ledger::RefExpr::Release(release) = ref_expr {
        ...
        let current_slot_ids: Vec<SlotId> = config
            ...
            .collect();
        crate::deploy::groups::validate_direct_release_membership(
            target_name,
            release,
            &rec,
            &current_slot_ids,
...
/// ([`crate::deploy::push::push`] step 2). An advisory lock is released by
...
/// [`crate::deploy::commit`] after the mutation loop) carries the status,
...
pub(crate) mod tests {
    ...
    //! end-to-end through [`crate::deploy::push::push`] /
    //! [`crate::deploy::push::push_inner`]. The shared harnesses live in
