...
//! deployment pipeline as a THIN COORDINATOR over the phase modules:
//! selection normalization + ref parsing + locking
//! ([`crate::deploy::preflight`]), materialization / release identity /
//! planning / capacity + staging ([`crate::deploy::preflight::run_preflight`],
//! [`crate::deploy::preflight::persist_intent`]), the batched per-server
//! publication + failure-policy compensation ([`crate::deploy::execute`],
//! with the batch loop in [`crate::deploy::batching`] and the status
//! derivation in [`crate::deploy::failure`]), the commit markers / status
//! decision ([`crate::deploy::status`]), and the terminal finalization +
//! observed-refresh + step-17 maintenance ([`crate::deploy::commit`],
//! [`crate::deploy::maintenance`]). The dry-run branch renders its plan via
//! [`crate::deploy::dryrun`]; the behavior-coverage gate lives in
//! [`crate::deploy::coverage`].
//!
//! This module keeps only the genuinely-orchestration pieces: [`push`], the
//! test-only entry points ([`push_with_id`], [`push_ref_with_id`]), the
//! [`slot_vars`] template helper (consumed by [`crate::deploy::batching`] /
//! [`crate::deploy::failure`]), the shared [`PushContext`] the phase modules
//! take, and the ordering test (`clean_push_transition_sequence_and_outcomes`).
//! The round-5 split moved the pre-mutation phases into
//! [`crate::deploy::preflight`], the mutation phases into
//! [`crate::deploy::execute`], and the post-mutation phases into
//! [`crate::deploy::commit`]; the shared test fixtures live in
...
use crate::identity::{ArtifactRef, DeploymentId, GenerationId, OperationId, SlotId};
use crate::ledger::{
    self, DeploymentStatus, LedgerIntentReport, LedgerTerminal, PushRef, RefExpr,
    TerminalDisposition,
};
...
/// ([`crate::deploy::preflight`], [`crate::deploy::execute`],
/// [`crate::deploy::commit`]) consume, bundled so the phase functions have
...
pub(crate) struct PushContext<'a> {
    ...
    pub(crate) selection: &'a crate::deploy::selection::SlotSelection,
    ...
    /// [`crate::deploy::preflight::run_preflight`].
...
pub fn push(
    ...
    // (the gate lives in [`crate::deploy::preflight::gate_direct_release_membership`],
    ...
    super::preflight::gate_direct_release_membership(store, config, target_name, &ref_expr)?;
    ...
    //    see [`crate::deploy::preflight::acquire_locks`] for the durable
    ...
    //    `push_inner` resolves.
    let (local_guard, target_guard) =
        super::preflight::acquire_locks(store, target_name, &op_id, opts.dry_run)?;
...
pub(crate) fn push_inner(
    ...
    // [`crate::deploy::preflight::PreflightOutcome`]).
    ...
    // [`crate::deploy::preflight`]: the read-only remote construction +
    // status ([`open_remotes`](crate::deploy::preflight::open_remotes) /
    // [`inspect_remotes`](crate::deploy::preflight::inspect_remotes)),
    ...
    super::preflight::open_remotes(&ctx, &mut remotes)?;
    super::preflight::inspect_remotes(&ctx, &remotes, &mut helpers, &mut statuses)?;
    let preflight = super::preflight::run_preflight(&ctx, &remotes, &helpers, &statuses)?;

    // ---- Dry-run: read-only planning, no mutation of store/remote/locks -----
    if opts.dry_run {
        // The dry-run PLAN RENDERING lives in [`crate::deploy::dryrun`]: the
        ...
        // is pure plan data.
        let msg = super::dryrun::render_dry_run_plan(
            store,
    ...
    // [`crate::deploy::preflight::persist_intent`].
    let attempt_intent = super::preflight::persist_intent(&ctx, &preflight)?;
    ...
    // (see [`crate::deploy::preflight::run_capacity_and_staging`]).
    if let Err(failure) = super::preflight::run_capacity_and_staging(
        store,
        &preflight.assignments,
        &helpers,
        op_id,
    ...
    // MUTATION PHASES (steps 10-15) in [`crate::deploy::execute`]: the
    ...
    // ACTUAL observation.
    let execution =
        super::execute::run_execution(&ctx, &preflight, &members, &remotes, &helpers, &statuses)?;

    // POST-MUTATION PHASES (steps 16-17) in [`crate::deploy::commit`]: the
    ...
    // assembly.
    super::commit::run_commit(&ctx, &attempt_intent, &execution, &members, &helpers)
}
