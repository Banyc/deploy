...
//!   ([`crate::deploy::batching`]) per the round-4 split.
...
//! observation ([`crate::deploy::failure::record_never_advanced_outcomes`])
//! stays with the failure-policy pass in [`crate::deploy::failure`], where
...
use crate::ledger::{
    Observation, ObservationError, ObservedGeneration, SlotAttemptState, SlotOutcomeKind,
    SlotResult,
};
