...
/// members are derived from the slots' `target` field). The unit artifact
...
# bound to target `production` by its `target` field).
#
...
# project's deploy_dir, and its `target` field binds it to the `production`
# target (a target's members are derived from the slots' `target` fields).
