//! The core wire + domain RECORD types of the deployment ledger (feature
...
//! [`DeploymentPlanWire`]) with their VERIFYING CONVERSIONS (`Wire::into_domain`),
//! the validated domain records ([`DeploymentIntent`], [`LedgerTerminal`],
//! [`LedgerRollback`], [`LedgerIntentReport`], [`DeploymentPlan`]), the
//! per-slot tables ([`SlotTable`] / [`NonEmptySlotTable`]), the observation
//! types ([`Observation`] and friends), and the accessors
//! ([`LedgerTerminal::outcomes`], [`LedgerTerminal::remaining_changes`],
//! [`LedgerTerminal::compensation`]). The MEMBERSHIP-EQUATION enforcement of
//! the `Successful` disposition lives in [`crate::ledger::membership`]; the
//! ledger's two physical line kinds + merged entry live in
//! [`crate::ledger::append`]; the rollback PAYLOAD builder lives in
//! [`crate::ledger::rollback`].
//!
...
//! kinds and the merged entry are [`crate::ledger::append::LedgerLine`] /
//! [`crate::ledger::append::LedgerEntry`], owned by [`crate::ledger::append`]
//! (which also documents the crash-atomic append + deployment-id keying
//! contracts). In brief: [`crate::ledger::append::LedgerLine::Intent`] is the
//! DURABLE INTENT of one deployment
...
//! [`crate::ledger::append::LedgerLine::Terminal`] is the TERMINAL EVENT of
//! one deployment
...
//!
//! A merged [`crate::ledger::append::LedgerEntry`] (intent + optional
//! terminal) is the deployment's
...
/// mutation. Appended once to the target's ledger ([`crate::ledger::append::LedgerLine::Intent`])
