//! Generation records (feature area A2).
//!
//! The remote generation record (`generations/<gen>/assignment.json`): its
//! [`GenerationAssignment`] struct, the `read_assignment` read path, and the
//! create-or-compare `create_generation` write that also installs the
//! generation `root` symlink. The record is immutable: a generation ID
//! colliding with different content fails integrity instead of silently
//! rewriting history.
