...
//!
//! One feature, one module: canonical tree objects ([`canonicalize_tree`],
//! [`compute_tree_digest`], [`entry_paths`]) and mapping/template
//! materialization ([`materialize_variant`], [`TemplateVars`],
//! [`render_template`], [`render_argv`]). A tree is immutable filesystem
//! content identified only by its digest; the canonical form records
//! normalized relative paths, entry types, modes, and content digests,
//! deliberately excluding ownership, timestamps, ACLs, and extended
//! attributes (stripped in schema version 1).
//!
