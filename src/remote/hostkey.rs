//! Host-key verification and pinning for the SSH transport.
//!
//! A fingerprint-only transport verifies the host key out-of-band BEFORE
//! first contact and pins it in a managed known-hosts file: the key is
//! fetched with `ssh-keyscan` and its fingerprint compared to the configured
//! value ([`pin_known_hosts`]). A pre-provisioned `known_hosts` file is used
//! as-is with `StrictHostKeyChecking=yes`. There is never trust-on-first-use:
//! if no host identity is configured the transport refuses to connect.
//!
//! Pinned keys live in a private (0700) cache directory owned by this user —
//! `$TMPDIR/deploy-ssh-knownhosts` by default, overridable via
//! `DEPLOY_SSH_KNOWNHOSTS_DIR` (tests use the override to isolate each test's
//! cache) — rather than a predictable world-readable temp file name, so a
//! locally pre-created file cannot be trusted blindly.
//!
//! The `ssh-keygen -lf` fingerprint check ([`key_matches_fingerprint`]) spawns
//! a LOCAL one-shot utility that reads a single key line from stdin and prints
//! its fingerprint — it performs no ssh/remote operation and cannot hang the
//! transport, so it deliberately stays OUTSIDE the bounded runner's scope.
//!
//! The `ssh-keyscan` fetch, by contrast, runs through THE shared bounded
//! runner ([`crate::remote::runner`]): the pin is bounded at the process
//! level by the connect deadline, and on deadline the child is killed and
//! reaped.
...
mod tests {
    use super::*;
    use crate::remote::runner::SSH_CONNECT_TIMEOUT_SECS;
