//! Commit markers (feature area A2).
//!
//! The idempotent per-server commit record
//! (`commits/<deployment-id>.json`, created exclusively with create-or-
//! compare): it records the generation a slot committed and the full slot set
//! that participated, so a partial marker can never masquerade as a complete
//! commit and a concurrent/retried commit never corrupts a recorded fact.
