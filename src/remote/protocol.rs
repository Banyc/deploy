//! Protocol handshake (feature area A5).
//!
//! [`RemoteHelper::handshake`] negotiates the remote-state protocol version:
//! first contact records this client's `PROTOCOL_VERSION` under
//! `control/protocol.json` via exclusive create; every later contact reads it
//! back and refuses on any mismatch, so an old client can never drive a state
//! directory written by a newer one (and vice versa). The protocol-version
//! constant itself lives with the transport layer
//! ([`crate::remote::transport::PROTOCOL_VERSION`]); the handshake methods
//! live here.
