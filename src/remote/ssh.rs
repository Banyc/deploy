...
//! verified out-of-band before first contact and pinned in a managed
//! known-hosts file (see [`crate::remote::hostkey`]). It never falls back to
//! trust-on-first-use: if no host identity is configured the transport refuses
//! to connect.
//!
//! Every ssh operation runs through the ONE bounded subprocess runner
//! ([`crate::remote::runner`]): hard deadline, kill, and deterministic reap,
//! so nothing is unbounded after connection establishment.
//!
...
impl SshTransport {
    ...
    /// it in a managed known-hosts file (see [`crate::remote::hostkey`]).
    ...
    /// lock; the verification/cache logic itself lives in `hostkey::pin_known_hosts`.
    pub(crate) fn pin_known_hosts(&self) -> Result<()> {
        let fingerprint = self
            .host_key_fingerprint
            .clone()
            .ok_or_else(|| Error::transport("host_key_fingerprint required for pinning"))?;
        let pinned = crate::remote::hostkey::pin_known_hosts(
            &fingerprint,
            &self.target,
            &self.address,
            self.port,
