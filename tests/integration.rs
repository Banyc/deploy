...
/// The deployment id of the LAST successful snapshot on `target` — the
...
/// remote root at all.
#[test]
fn rollback_refuses_rebound_slot() -> Result<()> {
    let tmp = tempfile::tempdir().unwrap();
    let proj = tmp.path().join("proj");
    std::fs::create_dir_all(&proj).unwrap();
    let store_base = tmp.path().join("store");
...
"#;
    ...
    assert!(
        !s02.exists(),
        "server-02 must not even be provisioned by the refused rollback"
    );
    assert!(
        !s02.join("current").exists() && !s02.join("generations").exists(),
        "no generation/current state may exist on server-02's remote root"
    );
    assert!(
    ...
    // config is valid; only EXACT rollback refuses).
    let r1 = push(
        &config_path,
        &store,
        &factory,
    ...
    assert_eq!(
        r1.status,
        Some(DeploymentStatus::Successful),
        "HEAD push on the rebound binding deploys to the new server"
    );
    assert!(
...
"#;
    ...
    let store = LocalStore::with_base(store_base.clone())?;
    let factory_base = remotes_base.clone();
    // The transport root models the PHYSICAL deployment location: the
    // server's directory AND the slot's deploy_dir (like the real
    // transports, whose root IS the deploy_dir) — so the deploy_dir's
    // IMMUTABLE receiver UUID lives per deploy_dir, and moving the
    // deploy_dir provisions a NEW physical location with a NEW receiver.
    let factory = move |s: &deploy::config::ServerDef,
        ...
        Ok(Box::new(LocalTransport::new(
            ...
            factory_base
                .join(&s.id)
                .join(slot.deploy_dir().to_string_lossy().trim_start_matches('/')),
        )?))
    };
    ...
    assert!(
        remotes_base
            .join("server-01/srv/move/movedir-a/current")
            .exists(),
        "server-01 hosts the deployed slot after s0"
    );
    ...
    // location (the transport root is the deploy_dir, so the moved
    // deploy_dir is a NEW physical location with a NEW receiver UUID — the
    // refused rollback must leave both locations untouched).
    let root_a = remotes_base.join("server-01/srv/move/movedir-a");
    let root_b = remotes_base.join("server-01/srv/move/movedir-b");
    ...
    let gens_before = gen_count(&root_a);
    let current_before = current_target(&root_a);
    assert!(
        gens_before >= 1,
        "the first deployment minted at least one generation"
    );
    let dep0 = latest_deployment_id(&store, "production");
    ...
    assert_eq!(
        gen_count(&root_b),
        1,
        "the HEAD push mints exactly one new generation on the moved location"
    );
    assert_ne!(
        current_target(&root_b),
        current_before,
...
impl SpyRemote {
    fn build(base: std::path::PathBuf, mutations: Arc<AtomicUsize>) -> Result<Box<dyn Remote>> {
        Ok(Box::new(SpyRemote {
            inner: LocalTransport::new(&deploy::env::SysEnv::from_process(), base)?,
            mutations,
        }))
    }
}
...
fn historical_rollback_uses_historical_behavior() -> Result<()> {
    ...
    // Change the configuration to behavior B, then roll back to the first
    // deployment (keyed by its deployment id).
    let config_b = setup_single(&proj, "false", true, 1);
    let rrb = push(
        &proj.join("deploy.toml"),
        &store,
...
fn incomplete_historical_behavior_fails_preflight_without_remote_mutation() -> Result<()> {
    ...
    // closed in preflight, NOT fall back to B.
    let dep0 = latest_deployment_id(&store, "production");
    let config_b = setup_single(&proj, "false", true, 1);
    let rrb = push(
        &proj.join("deploy.toml"),
...
/// `push production <deployment-id>` (the deployment-keyed rollback ref —
/// rollback payloads are keyed by deployment id) must still assign p1 to
/// `standard` (with the old tree) — the pre-refactor code resolved the
/// variant from the current config and failed with `release ... lacks
/// variant 'canary'`.
#[test]
...
"#
    ...
    let dep0_prod = latest_deployment_id(&store, "production");
    let dep0_staging = latest_deployment_id(&store, "staging");
    let rrb_prod = push(
        &proj.join("deploy.toml"),
        &store,
        &factory,
...
fn jj_style_refs_roll_back_along_snapshot_chain() -> Result<()> {
    ...
    // Deployment-keyed refs resolve to exact deployments: `deploys[3]--` =
    // two back = deploys[1], `parent(deploys[1], 1)` = deploys[0], a bare
    // deployment id = that deployment, and `parent(deploys[1], 0)` = itself.
    // (The release-refid and sN forms are REMOVED from the grammar.)
    let refids: [(String, usize); 4] = [
        (format!("{}--", deploys[3].as_str()), 1),
        (format!("parent({}, 1)", deploys[1].as_str()), 0),
        (format!("parent({}, 0)", deploys[1].as_str()), 1),
        (deploys[0].as_str().to_string(), 0),
    ];
    ...
    // Out-of-range ancestor walks fail closed as ref errors (the sN legacy
    // forms are rejected at parse).
    for bad in [
        "parent(@, 99)",
        &format!("{}--", deploys[0].as_str()),
        "s0-",
        "parent(s99, 0)",
    ] {
        let err = push(
